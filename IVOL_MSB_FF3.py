...
TE = pd.read_excel(data_accnt, 'TE')[:-2]
...
del BV['세아제강지주']
for i in range(int(len(BV)/365)): # i= 16 years
    for m in range(365):
        n = i * 365
        q = (i+1) * 4
        BV[n+m:n+m+1] = TE[q-1:q].values
...
writer = pd.ExcelWriter('C:\\Users\\장연숙\\Documents\\Paper2\\IVOL_MSB_FF3.xlsx')
