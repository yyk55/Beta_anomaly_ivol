...
data_beta = pd.ExcelFile('C:\\Users\\장연숙\\Documents\\Paper2\\KOSPI_DD.xlsx')
...
#FF-3factors
dmktcap = pd.read_excel(data_beta, 'MKTCAP_D')[730:]
dmktcap = dmktcap.set_index(list(dmktcap.columns[[0]]))

#Size sorting (each month), mktcapc = mktcap[common]
dmktcapc_size = dmktcap.copy() #sorting stocks each month by marketcap(Size)
...
writer = pd.ExcelWriter('C:\\Users\\장연숙\\Documents\\Paper2\\IVOL_MSB.xlsx')
bmr.to_excel(writer, 'benchmark_adj_r')
writer.save()

'''IVOL'''
data_bmr = pd.ExcelFile('C:\\Users\\장연숙\\Documents\\Paper2\\IVOL_MSB.xlsx')
...
ivol_c = ivol[common]
ivol_c = ivol_c.dropna(axis=1) #deleting ivol nan values
...
#checking correlation btwn pre-beta and ivol
prebeta_c = prebeta[common]
rh = ivol_c.corrwith(prebeta_c)
rhm = rh.mean()
...
half = round(len(rank1) / 2)
six = round(len(rank1) / 6) 
PTF = []
CNT = []
PTFB = []
