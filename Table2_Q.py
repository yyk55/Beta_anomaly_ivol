...
# Discriptvie Statistics
discriptive = rs_ptfb.describe()
from scipy.stats import describe
disc = describe(rs_ptfb, axis=0)
df_disc = {'Observations': disc[0], 'Min': disc[1][0], 'Max': disc[1][1], 'Mean': disc[2], 'Variance': disc[3], 'Skweness': disc[4], 'Kurtosis': disc[5]}
df_discstat = pd.DataFrame(df_disc, index=['Lowest beta ptf', '2', '3', '4', 'Highest beta ptf']).T
...
pbeta5 = []
tvalues5 = []
for i in range (len(rs_ptfb.columns)): #i = 15 ptfs
        ...
        #palpha.append(pa)
        pbeta5.append(pb)
        tvalues5.append(ttest)
        est.summary()

b5 = pbeta5.copy()
df_ab5 = np.array(b5)
df_ab5 = pd.DataFrame(df_ab5, index=['Lowest','2','3','4','Highest'], columns=['All stocks']).T
df_ab5['Highest-Lowest'] = df_ab5.iloc[:,-1] - df_ab5.iloc[:,0]
