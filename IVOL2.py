# -*- coding: utf-8 -*-
"""
Created on Sat Oct 20 10:11:02 2018
...
'''IVOL'''
data_bmr = pd.ExcelFile('C:\\Users\\장연숙\\Documents\\Paper2\\IVOL.xlsx')
...
for t in range(len(prebetan)):
    ...
    bt = bt[0]
    interb = np.array([el for el in bt if el not in inter]).tolist()
    
    mt = misp_msrn.iloc[t:t+1,:].values.tolist() 
    mt = mt[0]
    interm = np.array([el for el in mt if el not in inter]).tolist()
    ...
    b1 = interb[0:quar]
    b2 = interb[quar:quar*2]
    b3 = interb[quar*2:quar*3]
    b4 = interb[quar*3:]
