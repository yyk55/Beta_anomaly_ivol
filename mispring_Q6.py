# -*- coding: utf-8 -*-
"""
Created on Fri Oct 26 12:34:13 2018
...
#R = pd.read_excel(data_market, 'M_returns(M,T)')[5:-6]
#R = R.set_index(list(R.columns[[0]]))

#MOM = []
#for i in range(int(len(R)-6-1)):
#	e = (R[i:i+1].values+R[i+1:i+2].values+R[i+2:i+3].values+R[i+3:i+4].values+R[i+4:i+5].values+R[i+5:i+6].values).tolist()	
#	MOM.append(e)
...
df_MOM = pd.DataFrame(flat_MOM, index= date_rngm, columns = labels)
...
R = pd.read_excel(data_market, 'M_returns(M,T)')[11:-6]
R = R.set_index(list(R.columns[[0]]))

STR = []
for i in range(int(len(R)-1)):
	e = (R[i:i+1].values).tolist()	
	STR.append(e)
	
flat_STR = [item for sublist in STR for item in sublist]
labels = list(R.columns)
date_rngm = pd.date_range(start='2003-01-31', end='2017-12-31', freq='M')
df_STR = pd.DataFrame(flat_STR, index= date_rngm, columns = labels)
df_STR = df_STR.drop(['KB금융', '신한지주', '하나금융지주', '우리은행', '기업은행', '미래에셋대우', '한국금융지주', 'NH투자증권', '삼성증권', 'BNK금융지주', '메리츠종금증권', '키움증권', 'DGB금융지주', '메리츠금융지주', 'JB금융지주', '유안타증권', '대신증권', '한국자산신탁', '광주은행', '우리종금', '신영증권', '한국전자금융', '한화투자증권', '이베스트투자증권', 'SK증권', '현대차증권', '유진투자증권', 'KTB투자증권', '부국증권', 'DB금융투자', '유화증권', '골든브릿지증권', '에이티넘인베스트', 'SV인베스트먼트', '푸른저축은행', '제주은행', '한양증권', 'DSC인베스트먼트', '대성창투', '티에스인베스트먼트', '제미니투자'], axis=1)

#without Nan
filtered_STR = df_STR.dropna(axis=1)
rank_STR = filtered_STR.copy().T

for i in range(len(rank_STR.columns)):
    rank_STR.iloc[:,i] = rank_STR.iloc[:,i].rank(pct=True)
Rank_STR = rank_STR.T

#with Nan
nan_STR = df_STR
r_STR = nan_STR.copy().T

for i in range(len(r_STR.columns)):
    r_STR.iloc[:,i] = r_STR.iloc[:,i].rank(pct=True)
R_STR = r_STR.T

'''MAX Effect'''
data_max = pd.ExcelFile('C:\\Users\\장연숙\\Documents\\Paper2\\KOSPI_D.xlsx')

dr = pd.read_excel(data_max, 'KOSPI_Ri_D')[1064:-31]
dr = dr.set_index(list(dr.columns[[0]])) / 100  # set the first column as index

MAX = [] 
for i in range(len(R_STR)):
    t = i * 30
    m = (dr[t:t+30].max().values).tolist()
    MAX.append(m)
	
#flat_MAX = [item for sublist in MAX for item in sublist]
labels = list(dr.columns)
date_rngm = pd.date_range(start='2003-01-31', end='2017-12-31', freq='M')
df_MAX = pd.DataFrame(MAX, index= date_rngm, columns = labels)
df_MAX = df_MAX.drop(['KB금융', '신한지주', '하나금융지주', '우리은행', '기업은행', '미래에셋대우', '한국금융지주', 'NH투자증권', '삼성증권', 'BNK금융지주', '메리츠종금증권', '키움증권', 'DGB금융지주', '메리츠금융지주', 'JB금융지주', '유안타증권', '대신증권', '한국자산신탁', '광주은행', '우리종금', '신영증권', '한화투자증권', 'SK증권', '현대차증권', '유진투자증권', 'KTB투자증권', '부국증권', 'DB금융투자', '유화증권', '골든브릿지증권', '제주은행', '한양증권'], axis=1)

#with Nan
nan_MAX = df_MAX
r_MAX = nan_MAX.copy().T

for i in range(len(r_MAX.columns)):
    r_MAX.iloc[:,i] = r_MAX.iloc[:,i].rank(pct=True)
R_MAX = r_MAX.T
...
fna_STR = R_STR.fillna(0).copy()
cnt_STR = fna_STR.where(fna_STR == 0, 1)

fna_MAX = R_MAX.fillna(0).copy()
cnt_MAX = fna_MAX.where(fna_MAX == 0, 1)

count =  cnt_ItoA + cnt_ROA + cnt_GPP + cnt_STR + cnt_MAX + cnt_MOM

# calculation of total mispricing measure
sum_misp = fna_ItoA + fna_ROA + fna_GPP + fna_STR + fna_MAX + fna_MOM
misp_msr = (sum_misp / count).dropna(axis=1)

writer6 = pd.ExcelWriter('C:\\Users\\장연숙\\Documents\\Paper2\\mispring_Q6.xlsx')
