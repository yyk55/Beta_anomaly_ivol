...
for t in range(len(prebetan)):
    ...
    msr3 = [item for sublist in misp_msrn.iloc[t:t+1,tri*2:tri*3].values.tolist() for item in sublist]
    msr4 = [item for sublist in misp_msrn.iloc[t:t+1,tri*3:].values.tolist() for item in sublist]
    
    b1 = [item for sublist in prebetan.iloc[t:t+1,0:tri].values.tolist() for item in sublist]
    b2 = [item for sublist in prebetan.iloc[t:t+1,tri:tri*2].values.tolist() for item in sublist]
    b3 = [item for sublist in prebetan.iloc[t:t+1,tri*2:tri*3].values.tolist() for item in sublist]
    b4 = [item for sublist in prebetan.iloc[t:t+1,tri*3:].values.tolist() for item in sublist]
...
df_cntm = np.array(df_cnt).reshape(4,4)
df_cntm = pd.DataFrame(df_cntm, index=['Lowest','2','3','Highest'], columns=['Underpriced','2','3','Overpriced']).T


ri_kospis = ri_kospi[36:]
mktcap_kospis = mktcap_kospi[36:]
rs_ptf = prebetan.iloc[:,:16].copy().T # 15ptf returns
...
mktcap_kospic = mktcap_kospi[common]
ri_kospic = ri_kospi[common]
r_kospic = r_kospi.copy().T
for i in range(len(ri_kospic)): # t = 180
    rs = []
    r_ptf = ri_kospic.iloc[i:i+1]
    mktcap_ptf = mktcap_kospic.iloc[i:i+1]
    tmktcap = mktcap_ptf.sum().sum()
    r = (r_ptf * mktcap_ptf / tmktcap).sum().sum()
    rs.append(r)
    r_kospic.iloc[:,i] = rs
...
pb = df_cnt.copy()
pb[:] = pbeta
df_pbm = np.array(pb).reshape(4,4)
df_pbm = pd.DataFrame(df_pbm, index=['Lowest','2','3','Highest'], columns=['Underpriced','2','3','Overpriced']).T
...
a = alpha.copy()
df_a = np.array(a).reshape(4,4)
df_a = pd.DataFrame(df_a, index=['Lowest','2','3','Highest'], columns=['Underpriced','2','3','Overpriced']).T
df_a['Highest-Lowest'] = df_a.iloc[:,-1] - df_a.iloc[:,0]

p = [ps[i][0] for i in range(len(ps)) ].copy()
df_p = np.array(p).reshape(4,4)
df_p = pd.DataFrame(df_p, index=['Lowest','2','3','Highest'], columns=['Underpriced','2','3','Overpriced']).T

t = [ts[i][0] for i in range(len(ts)) ].copy()
df_t = np.array(t).reshape(4,4)
df_t = pd.DataFrame(df_t, index=['Lowest','2','3','Highest'], columns=['Underpriced','2','3','Overpriced']).T

#Regression for calculating t-value of the diffrence btwn hihg and low of 20 ptfs
alphat = []
tst = []
pst = []
for i in range (int(len(rs_ptf.columns)-12)):
        y = pd.DataFrame((rs_ptf.iloc[:,i+12].values - rfc.iloc[:,0].values)-(rs_ptf.iloc[:,i].values - rfc.iloc[:,0].values))
        #y = rs_ptf.iloc[:,i] - RF.iloc[:,0]
        x = pd.DataFrame([mkexr.iloc[:,0].values, SMB.iloc[:,0].values, HML.iloc[:,0].values], index = ['MKT', 'SMB', 'HML']).T
        x = sm.add_constant(x)
        est = sm.OLS(y,x).fit()
...
b = alphab.copy()
df_ab = np.array(b)
df_ab = pd.DataFrame(df_ab, index=['Lowest','2','3','Highest'], columns=['All stocks']).T
df_ab['Highest-Lowest'] = df_ab.iloc[:,-1] - df_ab.iloc[:,0]

pb = [psb[i][0] for i in range(len(psb)) ].copy()
df_pb = np.array(pb)
df_pb = pd.DataFrame(df_pb, index=['Lowest','2','3','Highest'], columns=['All stocks']).T

tb = [tsb[i][0] for i in range(len(tsb)) ].copy()
df_tb = np.array(tb)
df_tb = pd.DataFrame(df_tb, index=['Lowest','2','3','Highest'], columns=['All stocks']).T
