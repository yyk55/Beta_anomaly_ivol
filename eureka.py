...
data_kospi = pd.ExcelFile('C:\\Users\\장연숙\\Documents\\Paper2\\KOSPI200D.xlsx')

ri_kospi = pd.read_excel(data_kospi, 'KOSPI200_Ri(D)')
r_kospi = pd.read_excel(data_kospi, 'MKT200_R(D)')
r_kospim = pd.read_excel(data_kospi, 'KOSPI200_M')
...
writer = pd.ExcelWriter('C:\\Users\\장연숙\\Documents\\Paper2\\prebeta_d.xlsx')
...
data_prebeta = pd.ExcelFile('C:\\Users\\장연숙\\Documents\\Paper2\\prebeta_d.xlsx')
...
ri_kospis = ri_kospi[24:]
#mktcap_kospis = mktcap_kospi[1095:]
rs_ptf = prebetan.iloc[:,:4].copy().T # beta sorted 4ptf returns
for i in range(len(PTF)): # t = 180
    rs = []
