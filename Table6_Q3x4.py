...
data_mispmsr = pd.ExcelFile('C:\\Users\\장연숙\\Documents\\Paper2\\mispring_Q.xlsx')
...
df_cntm = np.array(df_cnt).reshape(3,4)
df_cntm = pd.DataFrame(df_cntm, index=['L ivol','2','H ivol'], columns=['underpricied','2','3','Overpriced']).T


ri_kospis = ri_kospi[36:]
mktcap_kospis = mktcap_kospi[36:]
rs_ptf = prebetan.iloc[:,:12].copy().T # 15ptf returns
...
rs_ptfvol = prebetan.iloc[:,:3].copy().T # 5ptf returns
...
pb = df_cnt.copy()
pb[:] = pbeta
df_pbm = np.array(pb).reshape(3,4)
df_pbm = pd.DataFrame(df_pbm, index=['L ivol','2','H ivol'], columns=['underpricied','2','3','Overpriced']).T
...
a = alpha.copy()
df_a = np.array(a).reshape(3,4)
df_a = pd.DataFrame(df_a,index=['L ivol','2','H ivol'], columns=['Underpricied','2','3','Overpriced']).T
...
#b = alphab.copy()
#df_ab = np.array(b)
#df_ab = pd.DataFrame(df_ab, index=['underpricied','2','3','Overpriced'], columns=['All stocks']).T
#df_ab['Overpriced-Underpriced'] = df_ab.iloc[:,-1] - df_ab.iloc[:,0]
#
#pb = [psb[i][0] for i in range(len(psb)) ].copy()
#df_pb = np.array(pb)
#df_pb = pd.DataFrame(df_pb, index=['underpricied','2','3','Overpriced'], columns=['All stocks']).T
#
#
#tb = [tsb[i][0] for i in range(len(tsb)) ].copy()
#df_tb = np.array(tb)
#df_tb = pd.DataFrame(df_tb, index=['underpricied','2','3','Overpriced'], columns=['All stocks']).T
