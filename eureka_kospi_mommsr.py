...
for i in range(int(len(R-11-1))):
	...
	MOM.append(e)
#for i in range(int(len(R-11-1))):
#	e = (R[i+1:i+2].values+R[i+2:i+3].values+R[i+3:i+4].values+R[i+4:i+5].values+R[i+5:i+6].values+R[i+6:i+7].values+R[i+7:i+8].values+R[i+8:i+9].values+R[i+9:i+10].values+R[i+10:i+11].values+R[i+11:i+12].values).tolist()	
#	MOM.append(e)
	
#flat_MOM = [item for sublist in MOM for item in sublist]
#labels = list(R.columns)
#date_rngm = pd.date_range(start='2002-12-31', end='2017-12-31', freq='M')
#df_MOM = pd.DataFrame(flat_MOM[:-1], index= date_rngm, columns = labels)
