...
for t in range(len(prebetan)):
    msr1 = [item for sublist in misp_msrn.iloc[t:t+1,0:quar].values.tolist() for item in sublist]
    msr2 = [item for sublist in misp_msrn.iloc[t:t+1,quar:quar*2].values.tolist() for item in sublist]
    msr3 = [item for sublist in misp_msrn.iloc[t:t+1,quar*2:quar*3].values.tolist() for item in sublist]
    msr4 = [item for sublist in misp_msrn.iloc[t:t+1,quar*3:].values.tolist() for item in sublist]
    
    ivol1 = [item for sublist in ivol_n.iloc[t:t+1,0:quin].values.tolist() for item in sublist]
    ivol2 = [item for sublist in ivol_n.iloc[t:t+1,quin:quin*2].values.tolist() for item in sublist]
    ivol3 = [item for sublist in ivol_n.iloc[t:t+1,quin*2:quin*3].values.tolist() for item in sublist]
    ivol4 = [item for sublist in ivol_n.iloc[t:t+1,quin*3:quin*4].values.tolist() for item in sublist]
    ivol5 = [item for sublist in ivol_n.iloc[t:t+1,quin*4:].values.tolist() for item in sublist]

    msr_list = [msr1,msr2,msr3,msr4]
    ivol_list = [ivol1,ivol2,ivol3,ivol4, ivol5]
    #b_list =[b1,b2,b3,b4,b5]
    ptf = [list(set(j).intersection(i)) for i in ivol_list for j in msr_list]
    cnt = list(len(ptf[c]) for c in range(len(ptf)))
    PTF.append(ptf)
    CNT.append(cnt)
    PTFM.append(msr_list)
    PTFVOL.append(ivol_list)
    
df_cnt = round(pd.DataFrame(CNT).mean(axis=0))
df_cntm = np.array(df_cnt).reshape(5,4)
df_cntm = pd.DataFrame(df_cntm, index=['L ivol','2','3','4','H ivol'], columns=['underpricied','2','3','Overpriced']).T
...
rs_ptfb = prebetan.iloc[:,:4].copy().T # 5ptf returns
for i in range(len(PTFM)): # t = 180
    rs = []
    for j in range(len(PTFM[i])):  #5 ptfs
        r_ptfb = ri_kospis[PTFM[i][j]][i:i+1]
#        mktcap_ptf = mktcap_kospis[PTFM[i][j]][i:i+1]
#        tmktcap = mktcap_ptf.sum().sum()
#        r = (r_ptfb * mktcap_ptf / tmktcap).sum().sum()
        r = r_ptfb.sum().sum() / len(PTFM[i][j])
        rs.append(r)
    rs_ptfb.iloc[:, i] = rs
rs_ptfb = rs_ptfb.T
rs_ptfb.describe()

rs_ptfvol = prebetan.iloc[:,:5].copy().T # 5ptf returns
...
df_pbm = pd.DataFrame(df_pbm, index=['L ivol','2','3','4','H ivol'], columns=['underpricied','2','3','Overpriced']).T
...
df_a = pd.DataFrame(df_a,index=['L ivol','2','3','4','H ivol'], columns=['Underpricied','2','3','Overpriced']).T
df_a['H ivol-L ivol'] = df_a.iloc[:,-1] - df_a.iloc[:,0]
#df_a.loc['Overpriced - Underpriced'] = df_a.loc['Overpriced'] - df_a.loc['Underpricied']

p = [ps[i][0] for i in range(len(ps)) ].copy()
df_p = np.array(p).reshape(5,4)
df_p = pd.DataFrame(df_p, index=['L ivol','2','3','4','H ivol'], columns=['Underpricied','2','3','Overpriced']).T

t = [ts[i][0] for i in range(len(ts)) ].copy()
df_t = np.array(t).reshape(5,4)
df_t = pd.DataFrame(df_t, index=['L ivol','2','3','4','H ivol'], columns=['Underpricied','2','3','Overpriced']).T
...
at = alphat.copy()
df_at = np.array(at)
df_at = pd.DataFrame(df_at, index=['underpricied','2','3','Overpriced'], columns=['H ivol-L ivol'])

pt = [pst[i][0] for i in range(len(pst)) ].copy()
df_pt = np.array(pt)
df_pt = pd.DataFrame(df_pt, index=['underpricied','2','3','Overpriced'], columns=['H ivol-L ivol'])

tt = [tst[i][0] for i in range(len(tst)) ].copy()
df_tt = np.array(tt)
df_tt = pd.DataFrame(df_tt, index=['underpricied','2','3','Overpriced'], columns=['H ivol-L ivol'])
...
#att = alphatt.copy()
#df_att = np.array(att)
#df_att = pd.DataFrame(df_att, index=['L ivol','2','3','4','H ivol'], columns=['Highest-Lowest'])
#
#ptt = [pstt[i][0] for i in range(len(pstt)) ].copy()
#df_ptt = np.array(ptt)
#df_ptt = pd.DataFrame(df_ptt, index=['L ivol','2','3','4','H ivol'], columns=['Highest-Lowest'])
#
#ttt = [tstt[i][0] for i in range(len(tstt)) ].copy()
#df_ttt = np.array(ttt)
#df_ttt = pd.DataFrame(df_ttt, index=['L ivol','2','3','4','H ivol'], columns=['Highest-Lowest'])
...
#b = alphab.copy()
#df_ab = np.array(b)
#df_ab = pd.DataFrame(df_ab, index=['L ivol','2','3','4','H ivol'], columns=['All stocks']).T
#df_ab['Overpriced-Underpriced'] = df_ab.iloc[:,-1] - df_ab.iloc[:,0]
#
#pb = [psb[i][0] for i in range(len(psb)) ].copy()
#df_pb = np.array(pb)
#df_pb = pd.DataFrame(df_pb, index=['L ivol','2','3','4','H ivol'], columns=['All stocks']).T
#
#
#tb = [tsb[i][0] for i in range(len(tsb)) ].copy()
#df_tb = np.array(tb)
#df_tb = pd.DataFrame(df_tb, index=['L ivol','2','3','4','H ivol'], columns=['All stocks']).T

#Regression for calculating t-value of the diffrence btwn hihg and low of 5 ptfs
alphabt = []
tsbt = []
psbt = []
for i in range (int(len(rs_ptfb.columns)-3)):
        y = pd.DataFrame((rs_ptfb.iloc[:,i+3].values - rfc.iloc[:,0].values) - (rs_ptfb.iloc[:,i].values - rfc.iloc[:,0].values))
        #y = rs_ptf.iloc[:,i] - RF.iloc[:,0]
        x = pd.DataFrame([mkexr.iloc[:,0].values, SMB.iloc[:,0].values, HML.iloc[:,0].values], index = ['MKT', 'SMB', 'HML']).T
        x = sm.add_constant(x)
        est = sm.OLS(y,x).fit()
...
c = alphavol.copy()
df_avol = np.array(c)
df_avol = pd.DataFrame(df_avol, index=['L ivol','2','3','4','H ivol'], columns=['All stocks']).T
df_avol['H ivol - L ivol'] = df_avol.iloc[:,-1] - df_avol.iloc[:,0]
...
df_tvol = np.array(tc)
df_tvol = pd.DataFrame(df_tvol, index=['L ivol','2','3','4','H ivol'], columns=['All stocks']).T

#Regression for calculating t-value of the diffrence btwn hihg and low of 5 ptfs
alphact = []
tsct = []
psct = []
for i in range (int(len(rs_ptfvol.columns)-4)):
        y = pd.DataFrame((rs_ptfvol.iloc[:,i+4].values - rfc.iloc[:,0].values) - (rs_ptfvol.iloc[:,i].values - rfc.iloc[:,0].values))
        #y = rs_ptf.iloc[:,i] - RF.iloc[:,0]
        x = pd.DataFrame([mkexr.iloc[:,0].values, SMB.iloc[:,0].values, HML.iloc[:,0].values], index = ['MKT', 'SMB', 'HML']).T
        x = sm.add_constant(x)
        est = sm.OLS(y,x).fit()
